...
public class FeishuBot {
    ...
    public FeishuBot(AgentEngine engine) {
        this(engine, System.getenv("FEISHU_APP_ID"), System.getenv("FEISHU_APP_SECRET"));
    }

    public FeishuBot(AgentEngine engine, String appId, String appSecret) {
    ...
    public void handleAgentRun(String chatId, String prompt) {
        ...
        try {
            Session session = SessionManager.GLOBAL.getOrCreate(chatId, java.nio.file.Path.of(".").toAbsolutePath().normalize());
            session.append(com.bugwiki.harness.schema.Message.user(prompt));
