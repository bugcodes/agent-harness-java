...
public class AgentEngine {
    ...
    private void runInternal(String userPrompt, Object ignoredReporter) throws Exception {
        ...
        for (int turn = 1; turn <= 20; turn++) {
            ...
            System.out.println("[Engine] 模型请求调用 " + action.getToolCalls().size() + " 个工具...");
    ...
    private List<Message> executeCalls(List<ToolCall> calls) {
        ...
        for (ToolCall call : calls) {
            System.out.println("  -> 🛠️ 执行工具: " + call.getName() + ", 参数: " + call.getArguments());
            ToolResult result = registry.execute(call);
            String output = result.getOutput();
            if (result.isError()) {
                System.out.println("  -> ❌ 工具执行报错: " + output);
            } else {
                System.out.println("  -> ✅ 工具执行成功 (返回 " + output.length() + " 字节)");
            }
