...
public class AgentEngine implements AgentRunner {
    ...
    private final Compactor compactor = new Compactor(200_000, 6);
    ...
    public void run(Session session, Reporter reporter) throws Exception {
        run(session, reporter, null);
    }

    public void run(Session session, Reporter reporter, TraceSpan parentSpan) throws Exception {
        System.out.println(
                "[Engine] 唤醒会话 ["
                        + session.getId()
                        + "]，锁定工作区: "
                        ...
                        + planMode
                        + ")");
        TraceSpan rootSpan = Tracer.startSpan(parentSpan, "Agent.Run");
        rootSpan.addAttribute("SessionID", session.getId());
        rootSpan.addAttribute("WorkDir", session.getWorkDir().toString());
        try {
            Message systemMsg = new PromptComposer(session.getWorkDir(), planMode).build();
            int turnCount = 0;
            while (true) {
                turnCount++;
                TraceSpan turnSpan = Tracer.startSpan(rootSpan, "Turn-" + turnCount);
                try {
                    List<Message> context = new ArrayList<>();
                    context.add(systemMsg);
                    List<Message> workingMemory = session.getWorkingMemory(20);
                    if (!workingMemory.isEmpty() && workingMemory.get(0).getRole() != Role.USER) {
                        workingMemory.add(
                                0,
                                Message.user(
                                        "[系统占位符] 这是为了保持上下文连贯性而注入的断点标记。请继续执行你刚才的任务。"));
                    }
                    context.addAll(workingMemory);
                    List<Message> compactedContext = new ArrayList<>(compactor.compact(context));
                    turnSpan.addAttribute("context_message_count", compactedContext.size());
                    ...
                    if (enableThinking) {
                        ...
                        TraceSpan thinkSpan = Tracer.startSpan(turnSpan, "LLM.Thinking");
                        Message thought;
                        try {
                            thought = provider.generate(compactedContext, null);
                        } finally {
                            thinkSpan.endSpan();
                        }
                    ...
                    TraceSpan actionSpan = Tracer.startSpan(turnSpan, "LLM.Action");
                    Message action;
                    try {
                        action = provider.generate(compactedContext, registry.getAvailableTools());
                    } finally {
                        actionSpan.endSpan();
                    }
                    ...
                    ExecutionBatch batch = executeCalls(action.getToolCalls(), reporter, turnSpan);
            ...
            rootSpan.endSpan();
            try {
                Tracer.exportTraceToFile(rootSpan, session.getWorkDir(), session.getId());
            } catch (Exception ignored) {
                // Go 版导出失败时不影响主流程。
            }
            System.out.println("📊 [Tracing] 本次任务的执行回放链路已保存至工作区的 .claw/traces 目录下");
        }
    }

    private ExecutionBatch executeCalls(List<ToolCall> calls, Reporter reporter, TraceSpan turnSpan) {
                ...
                futures.add(
                        CompletableFuture.supplyAsync(
                                () -> {
                                    ...
                                    ToolResult result = registry.execute(call, turnSpan, reporter);
